import { createPlaylistRequest as CreatePlaylistRequest, PopulateFavList, UpdatePlaylistRequest } from "#/@types/audio";
...
export const updatePlaylist: RequestHandler = async (req: UpdatePlaylistRequest, res) => {
    ...
    if(item){
        ...
        if(!audio) return res.status(404).json({error: "Không tìm thấy Audio!"})
        await Playlist.findByIdAndUpdate(playlist._id, {
            $addToSet: {items: item}
        })
    }
 
    res.status(201).json({Playlist: {id: playlist._id, title: playlist.title, visibility: playlist.visibility }})
};
...
export const getPlaylistByProfile: RequestHandler = async (req, res) => {
    ...
    const data = await Playlist.find({
        owner: req.user.id,
        visibility: {$ne: 'auto'}
    }).skip(parseInt(pageNo) * parseInt(limit)).limit(parseInt(limit)).sort('-createAt')
